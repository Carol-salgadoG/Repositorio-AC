...
# ╔═╡ 6541bc23-c891-4a31-86f4-e47bf5babbed
...
#Definimos la función y paramétros
begin
	function newton(f,f′,x_0,n)
	for i in n 													#Utilizaremos un for para realizar las iteraciones
	x=x_0-(f/f′) 												#Usaremos la fórmula del ejercicio anterior
		println("La raíz es ",x, " de ", i , " iteraciones") 	#Imprimimos la leyenda
	end
end
end

# ╔═╡ 134a2581-b0bc-4cac-a5a4-88060500f32b
newton(0.134,-8.15,0.5,100) 									#Realizamos la operación
...
begin
	function newtonRecursiva(f1,df,x0,n1) 						#Definimos la función newtonRecursiva
		if n1 >= 0 												#Usamos un if, que toma los valores mayor o igual a cero
	return x0-(f1/df)											 #Y devolverá el resultado, usando la misma operación
	end
end
end

# ╔═╡ 57804a1d-2743-48fd-adaf-52f9540492e4
newtonRecursiva(0.134,-8.15,0.5,100) 							#Realizamos la operación
...
begin
	function newtonDistancia(F,F′,X0,ϵ) 								#Definimos la función
		if ϵ<=0 														#Describimos la condición donde E mayor que cero
		println("Error, el valor de epsilon es menor o igual que cero") #Si es correcto, va a realizar el método newton 
		end
		X=X0-(F/F′)
		while abs(X-X0)<ϵ
	return X 															#Y devolverá el resultado
	end
end
end

# ╔═╡ 5114c096-2d06-42c7-a5e9-1a1efe8ef007
newtonDistancia(0.134,-8.15,0.5,100)
...
derivadaNumérica(F1,X,h)= (F1(X+h) - F1(X))/h 			#Usando el hint, usando la sintaxis nombre(parámetro1, parámetro2,) = expresión, definimos con la aproximación que vimos anteriormente

# ╔═╡ 731def47-b524-44e2-beb7-ed88acd6261f
derivadaNumérica(cos, 4, 20)  							#Escribimos la derivada
...
newtonDN(F1,X0,H)=X0 - F1(X0)/derivadaNumérica(F1, X0, H) 		#Definimos la función y sus parametros
...
* `h` en `1000:1000:1000000` para la proximación de las derivadas numéricas como la ecuación (*);
* `n` en `1:1000` para el numero de interaciones del metodo de Newton;
...
# ╔═╡ eace0db8-9658-479c-911c-405909284998
begin
...
# ╔═╡ 394eee52-808a-4bff-a932-44c2df059652
# Tu código (comentado) va aquí :D
	#Vamos a intentarlo ^^
	
	#Definimos la funcion con las variables que nos piden para calcular las aproximaciones 
function newtonDN(f, df, x0, n)
    x = [x0] 									#Definimos a x con su rango
    for i in 1:n
        xi = x[i] - f(x[i]) / df(x[i]) 			#Así como su definición de esta 
        push!(x, xi)
    end
...
newtonDN(cos, 4, 20) 											#Escribimos la derivada

# ╔═╡ 16515ebf-ebb4-454e-9dc2-d65ee7222726
function plot_graph(f, x0_range, h_range, n_range) 						#Nos permite para calcular y graficar puntos
    x_values = collect(x0_range)
    
    for h in h_range
        for n in n_range
            y_values = newtonDN(f, sin, x_values[1], n)  				#Utilizan ciclos for que calcula y grafica la aproximación de f(x) para los puntos
            
            for i in 2:length(x_values)
                x = [x_values[i-1], x_values[i]] 						#Definimos a x y a y
                y = [f(x_values[i-1]), f(x_values[i])]
                plot!(x, y, marker = true, label = "h = $h, n = $n")
            end
        end
    end
...
# ╔═╡ e47ff8b8-c47b-472a-9bd0-8980eac89e33
begin 																#Aqui esta la grádica con el rango que tiene
x0_range = -10:0.1:10
h_range = 1000:1000:1000000
n_range = 1:1000

plot_graph(cos, x0_range, h_range, n_range)
end
#Veremos como cambia a continuación
