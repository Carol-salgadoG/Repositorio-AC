...
#Vamos a intentarlo ^^ Este tambien estuvo dificil )):
begin
	#Tenemos que usar Statistics para que duncione esta animación
	using Statistics
	
	#Vamos a definir la animación
	function animaCaminata2D(caminata)
	    pasos_horizontales = caminata[1] 				#Con el paso horizontal
	    pasos_verticales = caminata[2] 					#Y vertical
	
	    n = length(pasos_horizontales) 					#Definimos el rango
	
	    anim = @animate for i in 1:n 					#Y definimos la animación
	        plot(pasos_horizontales[1:i], pasos_verticales[1:i], line=:dash, marker=:circle, aspect_ratio=:equal, legend=false)  #Definimos los aspectos de la animación
	        xlabel!("Posición Horizontal") 				#Así como los parámetros
	        ylabel!("Posición Vertical")
	        title!("Caminata Aleatoria en 2D") 			#Y titulo
	    end
	
	    return anim 									#Definimos la repetición
	end
	#Cada vez que corremos esta celda volvera con otra caminata aleatoria
end

# ╔═╡ 710df1bb-d7ed-4919-944a-72f97471ce04
md"Carol Michelle Salgado Gómez. Tarea"
...
# Decidi cambiar las variables porque el código no nos permite usar la misma variable a la vez
function verificarDistribucionUniforme(c, d, x)
    # Generamos una muestra de n números aleatorios
    muestra = [aleatorioUniforme(c, d) for _ in 1:x]

    # Y creamos un histograma de la muestra definida
...
# Verificamos la distribución uniforme y graficamos el histograma
...
# Usamos esta funcion para definir los pasos.
function unPaso()
    return 2 * rand(Bool) - 1
end

# Nota: no es necesario re escribirlo después, basta con definirlo una vez para que podamos usarlo.

# ╔═╡ 50eb09fa-eb61-4cca-a025-30b3223c3e09
# ╠═╡ disabled = true
#=╠═╡
resultado = unPaso() # Definimos el paso con el nombre de "resultado"
  ╠═╡ =#
...
function variosPasos(i) 						#Con una variable i
    return 2 .* rand(Bool, i) .- 1 				#Definimos ambas posibilidades
end

# ╔═╡ 58075114-240b-4baa-93da-787bc351c1ea
#En otra serie definimos el valor de la variable y vamos a imprimir el resultado
...
#En una celda diferente definimos la funcion paso
...
#Definimos la función de caminata
function caminataAleatoria(k)  							#Con una variable k
    pasos = [0]
    for _ in 1:k 										#De 1 a la variable
        paso = unPaso(k) 								#Definimos "peso" con la función 
        posicion_actual = pasos[end] 					#Y definimos la posición
        nueva_posicion = posicion_actual + paso 		#Y como cambia a cada paso
        push!(pasos, nueva_posicion) 					#Luego combinamos las funciones
    end
    return pasos
end
...
# ╠═╡ disabled = true
#=╠═╡
k = 10 													#Definimos la variable
  ╠═╡ =#

# ╔═╡ 303514f3-fc1f-46b9-9b0a-e4f0d9f6c61f
#=╠═╡
resultado2 = caminataAleatoria(k) 						#Definimos al resultado
  ╠═╡ =#

# ╔═╡ f44c4a85-acb9-4da3-ba1a-67e996b3c06c
#=╠═╡
println(resultado2) 									#Y lo imprimimos
  ╠═╡ =#
...
begin
#Como ya tenemos definida la función, vamos a definir la gráfica	
	function graficaCaminata(caminata)
	    n = length(caminata) 						#Definimos el largo de los pasos
	    pasos = 0:n-1 								#De 1 a n
	    scatter(pasos, caminata, xlabel = "Número de Pasos", ylabel = "Posición", color = "yellow", 								#Modificamos como se verá la gráfica, sus ejes y el color
	            title = "Caminata Aleatoria")       #Y el título
	end
...
#Generamos graficas adicionales de caminatas aleatorias
n2 = 200

# ╔═╡ 1144766d-2a54-4a1e-b3ef-931b80de76ab
caminata2= caminataAleatoria(n2) 					#Y definimos la camintas aleatorias

# ╔═╡ 32bc24c1-dbb9-401a-9066-e5d37e78ecca
begin
	graficaCaminata(caminata2) 						#Imprimimos la gráfica
end
...
begin
	...
	begin
	function graficaCaminata!(caminatas) 				#Definimos las gráficas de caminatas
		    n = size(caminatas, 2) 						#El tamaño de cada una
		    pasos = 0:n-1 								#Con el número de pasos
		    for i in 1:size(caminatas, 1) 				#Definimos de donde a donde
		        plot!(pasos, caminatas[i, :], linestyle=:dash, label="Caminata $i")
		    end
		    xlabel!("Número de Pasos") 					#Y editamos su visualización
		    ylabel!("Posición")
...
# Generamos las 5 caminatas aleatorias y las graficamos juntas
num_caminatas = 5

# ╔═╡ 5aaec01c-7eee-4be2-a19b-76ab2e3c4e56
# ╠═╡ disabled = true
#=╠═╡
n = 100 												#Definimos la variable
  ╠═╡ =#
...
begin
#Como ya habiamos definido con anterioridad la caminata, lo animamos
	function animaCaminata(caminata) 					#Definimos la animación
	    n = length(caminata) 							#Vemos la longitud que tendrá
	    pasos = 0:n-1 									#Con el número de pasos
	    
	    animation = @animate for i in 1:n 				#Definimos la animación de la gráfica
	        graficaCaminata(caminata[1:i]) 				#Y la gráfica de esta funcion
	    end
	    
	    gif(animation, "caminata_animada.gif", fps = 10) #Animamos en forma de gift
	end
end


# ╔═╡ a7de77db-2c8d-4b17-ae4a-3dd57c688535
caminatagif = caminataAleatoria(100) 					#Definimos al git con los datos anteriores

# ╔═╡ abff0a42-8c9e-4379-afa7-654403b08754
animaCaminata(caminatagif) 							#Animamos el grif y se guarda en nuestra memoria
...
# Vamos a intentarlo ^^ este si fue más largo ):

#Definimos la función de caminata
function caminataAleatoria2D(l)
    pasos_horizontales = [0] 					#Con el inicio de pasos horizontales
    pasos_verticales = [0] 						#Y verticales

    for _ in 1:l 								#Definimos el rango de trabajo con la variable
        paso_horizontal = round(aleatorioUniforme(-1, 1)) #Definimos cada paso horinzotal
        paso_vertical = round(aleatorioUniforme(-1, 1)) #Y vertical

        posicion_horizontal_actual = pasos_horizontales[end] #Con las posiciones de estos, sea horizontal
        posicion_vertical_actual = pasos_verticales[end] #O vertical

        nueva_posicion_horizontal = posicion_horizontal_actual + paso_horizontal #Conla definición anterior vimos como "actualizar" los pasos y repetimos aqui, por lo que lo hacemos para la posición horizontal
        nueva_posicion_vertical = posicion_vertical_actual + paso_vertical #Y vertical

        push!(pasos_horizontales, nueva_posicion_horizontal)
        push!(pasos_verticales, nueva_posicion_vertical)
    end

    return [pasos_horizontales, pasos_verticales] #Repetimos ambas definiciones
end

# ╔═╡ ab167566-c46a-4625-a5b9-78839ffec8d4
l = 10 												#Definimos la variable

# ╔═╡ ebd81109-acce-4126-9c5d-415146e794ed
resultado = caminataAleatoria2D(l) 					#Definimos al resultado

# ╔═╡ a841ff20-ff2f-4d14-954c-e12be9d428f1
println(resultado) 			#Imprimimos el resultado

# ╔═╡ f01c93dd-537f-4701-9685-38367d1798b2
println(resultado) 									#Y lo imprimimos
...
begin
	#Volvemos a definir la gráfica de la caminata
function graficaCaminata2D(caminata)
	    pasos_horizontales = caminata[1] 				#Con los pasos horizontales
	    pasos_verticales = caminata[2] 					#Y verticales
	
	    plot(pasos_horizontales, pasos_verticales, line=:dash, marker=:circle, aspect_ratio=:equal, legend=false) 					#Definimos los marcos de la gráfica
	    xlabel!("Posición Horizontal") 					#Y los ejes de esta
	    ylabel!("Posición Vertical")
	    title!("Caminata Aleatoria en 2D") 				#Con el título
	end
...
begin
	n3= 10; 									#Definimos una variable aleatoria
	Caminata = caminataAleatoria2D(n3) 			#Definimos "Caminata" con lo anterior
	graficaCaminata2D(Caminata) 				#Y gráficamos
end
...
function graficaCaminata2D!(p, caminata) 					#Como habiamos definido la caminata antes, reusamos esto
    pasos_horizontales = caminata[1] 			#Y definimos los pasos horizontales
    pasos_verticales = caminata[2] 				#Y los pasos verticales

    plot!(p, pasos_horizontales, pasos_verticales, line=:dash, marker=:circle)
end

# ╔═╡ 9dfee2f2-b559-4196-a4a5-72820ea9d006
p = plot(xlabel="Posición horizontal", ylabel="Posición vertical", title="Caminatas aleatorias en 2D", aspect_ratio=:equal, legent=false) #Definimos a la gráfica p

# ╔═╡ 5af465eb-8fca-43c2-bae8-43c9218150eb
for _ in 1:5 										#Y definimos el rango
	caminata = caminataAleatoria2D(100) 			#Con la caminata aleatoria
	graficaCaminata2D!(p, caminata) 				#Y graficamos
end
...
begin
	caminata = caminataAleatoria2D(100) 				#Definimos la caminata con lo anterior
	anim = animaCaminata2D(caminata) 					#Y animamos 
end

# ╔═╡ 65a0036e-763f-431b-8e61-ee193815b29f
gif(anim, "caminata.gif") 						#Guardamos el gif en nuestra memoria
...
# Vamos a intentarlo ^^ Este también estuvo pesado )):

#Definimos la funcion de la caminata
function caminataAleatoria3D(n) 					#Al ser en 3D, tenemos que definir cada coordenada 
    pasos_x = [0.0]
    pasos_y = [0.0]
    pasos_z = [0.0]

    for _ in 1:n 									#Definimos el rango de cada paso del eje
        paso_x = rand([-1, 1])
        paso_y = rand([-1, 1])
        paso_z = rand([-1, 1])
														#Definimos su posición de cada eje
        posicion_x_actual = pasos_x[end]
        posicion_y_actual = pasos_y[end]
        posicion_z_actual = pasos_z[end]
														#Y volvemos a definir las nuevas posiciones como la anterior vez, pero para los 3 ejes
        ...
        nueva_posicion_z = posicion_z_actual + paso_z
									#Definimos la secuencia que iran
        push!(pasos_x, nueva_posicion_x)
        push!(pasos_y, nueva_posicion_y)
        push!(pasos_z, nueva_posicion_z)
    end

    return [pasos_x, pasos_y, pasos_z] 				#Y hacemos que se repita nuevamente el código
end

#Si corremos esta celda se repetira aleatoriamente!

# ╔═╡ 510f7147-5a2b-4ecc-866f-166ffdd326c8
function graficaCaminata3D(caminata) 			#Definimos la grafica en 3D
    pasos_x = caminata[1] 						#Recordando que se tiene que definir los pasos en cada eje
    pasos_y = caminata[2]
    pasos_z = caminata[3]

    plot3d(pasos_x, pasos_y, pasos_z, line=:dash, marker=:circle, legend=false) #Vamos a definir como se visualizara la gráfica
    xlabel!("Posición X") 						#Definimos el nombre de las posiciones
    ylabel!("Posición Y")
    zlabel!("Posición Z")
    title!("Caminata Aleatoria en 3D") 			#Describimos el titulo
end
...
caminatas = zeros(num_caminatas, n+1) 					#Y definimos la matriz
#Cada vez que reiniciemos la matriz se generaran otras

# ╔═╡ 7d6df278-83fa-4ee3-b04f-aba267080b17
graficaCaminata!(caminatas) 							#Y lo graficamos

# ╔═╡ 6b9b5846-038d-408a-abc2-750519330526
for i in 1:num_caminatas 								#De una al numero de caminatas que realicemos
	caminatas[1, :] = caminataAleatoria(n) 				#Se definira "caminatas"
end

# ╔═╡ 213112db-7670-46ac-be4a-4f5b338b4d16
Caminata1 = caminataAleatoria3D(n) 				#Definimos las caminatas

# ╔═╡ 9cf1ea65-d3fb-40c5-a581-72ba1ec4411e
graficaCaminata3D(Caminata1) 					#Y graficamos la caminata
