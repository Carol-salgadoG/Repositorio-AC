...
@bind d Slider(0.0005:0.0005:1, default=0.1348)

# ╔═╡ 17368c42-44e5-4d4e-ac00-d0f2f65a2dd4
d

# ╔═╡ 9de58f5e-591a-45d6-b412-7b541ee17445
begin #Empezamos la función ^^
	plot(sin,-π:d:π, title = "Funciones trigonométricas", xlabel = "a", ylabel = "b", color = "red", label = "sin(x)", marker = false)  #Definimos el título, así como la primera gráfica de la función sen con el color rojo
	plot!(cos,-π:d:π, color = "purple", label = "cos(x)", marker = false) #Definimos la función contraria de color morado
end
...
al = 13				    				               	#Nombramos "al" la altura (porque h ya se había definido)
r = 8			    															        #Nombramos "r" la rapidez
θ = 45	       																	        #Nombramos "θ" el ángulo
t = 4     																		        #Nombramos "t" el tiempo
	
	Vrx = r * cos(θ)                                                                    #Nombramos el vector velocidad en el eje x
	Vry = r * sin(θ) 																	#Y el vector velocidad en el eje y	

p(x) = ((Vrx) * x, al + (Vry) * x - ((9.81 * x^2) / 2))                             #Nombramos el vector posición (9.81 por la gravedad)
...
scatter!([px(t)],[py(t)])                                                   #Graficamos la particula en el tiro parabolico
	...
	x_max = (Vrx * ((2 * Vry/9.81 + ((2 * Vry/9.81)^2 + 8 * al/9.81)^(1/2))/2))          #Calculamos su alcance máximo horizontalmente
	y_max = (al + (Vry) * (Vry/9.81) - 9.81 * (Vry/9.81)^2/2)                            #Calculamos su alcance máximo verticalmente
		
		xlabel!("Distancia")                                                #Nombramos "distancia" al eje x
		ylabel!("Altura")                                                   #Nombramos "altura" al eje y
		xlims!(0,x_max + 0.5)                                               #Definimos los límites de cada eje (x).
		ylims!(0,y_max + 0.5)                                               #Y el eje y.
		
end

	gif(tiro, "Tiro_Parabólico.gif", fps = 30)                              #Creamos la variable tiro, en forma de gif "Tiro Parabólico".
...
function parabola(al,r,θ,t)													#Nombramos la función de parametros al,r,θ,t
	
	Vrx = r * cos(θ)                                                        #Nombramos al vector velocidad en el eje x
	Vry = r * sin(θ) 														#Y al vector velocidad en el eje y	

p(x) = ((Vrx) * x, al + (Vry) * x - ((9.81 * x^2) / 2))                      #Describimos el vector posición.
...
scatter!([px(t)],[py(t)])                                                   #Graficamos la particula del tiro parabolico
	...
	x_max = (Vrx * ((2 * Vry/9.81 + ((2 * Vry/9.81)^2 + 8 * al/9.81)^(1/2))/2))          #Calculamos el alcance máximo horizontalmente
	y_max = (al + (Vry) * (Vry/9.81) - 9.81 * (Vry/9.81)^2/2)                            #Calculamos el alcance máximo verticalmente
		
		xlabel!("Distancia")                                               #Nombramos "distancia" al eje x
		ylabel!("Altura")                                                  #Nombramos "altura" al eje y
		xlims!(0,x_max + 0.5)                                              #Definimos límites del eje x
		ylims!(0,y_max + 0.5)                                              #Definimos límites del eje y
		
end

	gif(tiro, "Tiro_Parabólico_2.gif", fps = 30)                           #Creamos la variable tiro, en forma de gif "Tiro Parabólico".
...
@gif for i in range(0, stop = 2π, length = n)                    #Nombramos rango de la gráfica, y n definirá su "rápidez"
	s(x,y) = cos(x + 10sin(i)) + sin(y)                          #Nombramos la función s y se mueva en "y"

p = plot(x, y, s, st =:wireframe)                                #Gráficamos la función con variables sin definir
	
	xlabel!("x")                                                 #Nombramos "x" al eje x.
    ylabel!("y")                                                 #Nombramos "y" al eje y.
...
#Vemos que la gráfica se desplaza hacía el eje "y"
end
end
